...
# Hint: if you remove the "#" below, you can add the two functions you created into the user_rating_model dataset
```{r prepare the data}
user_rating_model <- df %>%
  ...
  select(user_rating_ver,size_bytes,price,price_degree,avg_rating_count_ver,rating_count_ver,sup_devices_num,cont_rating,
         ipad_sc_urls_num,lang_num,vpp_lic,cont_rating_degree)
...
user_rating_model %>%
  assert(within_bounds(lower.bound = 0, upper.bound = Inf, size_MB))
```


# Hint:Because the cont_rating_degree will only contain three values including "Low","Middle","High". Thus we need to test whether the cont_rating_degree column contains only the three values.
```{r test cont_rating_degree function}
user_rating_model %>% 
  assert(in_set(c("Low", "Middle", "High")), cont_rating_degree)
```
